...
func startWebServer() {
	...
	r.HandleFunc("/api/cart/{id}", deleteCartItem).Methods("DELETE")
...
func deleteCartItem(w http.ResponseWriter, r *http.Request) {
	...
	result, err := cartCollection.DeleteOne(context.TODO(), bson.M{"_id": objID})
	if err != nil {
...
func getProductByID(w http.ResponseWriter, r *http.Request) {
	...
	// Query MongoDB to find the product by ObjectID
	var product Product
	err = productsCollection.FindOne(context.TODO(), bson.M{"_id": objectID}).Decode(&product)
	if err != nil {
		http.Error(w, "Product not found", http.StatusNotFound)
		return
	}

	// Respond with product data in JSON format
